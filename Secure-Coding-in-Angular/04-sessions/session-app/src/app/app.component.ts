...
export class AppComponent implements OnInit, OnDestroy {
  ...
  isAuthenticated$: Observable<boolean> = this.authService.auth$.pipe(
    map((authState: AuthState) => authState.isAuthenticated)
  );
  // name of the authenticated user
  name$: Observable<string | undefined> = this.authService.auth$.pipe(
    map((authState: AuthState) => authState.name)
  );

  private authSub$!: SubscriptionLike;

  ngOnInit(): void {
    this.authSub$ = this.authService.auth$
      .pipe(
        skip(1),
